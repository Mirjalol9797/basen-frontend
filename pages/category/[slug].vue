...
const seoTextMap: Record<string, string> = {
  open: 'Открытые бассейны Ташкента — сезонные комплексы под открытым небом. Работают с мая по сентябрь. Здесь вы найдёте бассейны с солярием, летними террасами, ресторанами и детскими зонами. Идеальны для семейного отдыха в тёплое время года.',
  indoor: 'Крытые бассейны Ташкента работают круглый год вне зависимости от погоды. Оснащены системой фильтрации и подогрева воды. Подходят для регулярных тренировок, занятий с тренером и оздоровительного плавания.',
  children: 'Детские бассейны Ташкента созданы специально для малышей от 3 лет. Небольшая глубина, тёплая вода, игровые зоны и присмотр персонала обеспечивают безопасный отдых. Многие комплексы предлагают занятия с детским инструктором.',
  sport: 'Спортивные бассейны Ташкента — олимпийские дорожки, профессиональные тренеры и серьёзная инфраструктура. Подходят для подготовки к соревнованиям, любительского плавания и занятий ватерполо.',
  hotel: 'Бассейны при гостиницах Ташкента доступны как для постояльцев, так и для сторонних посетителей. Высокий уровень сервиса, ухоженная территория и дополнительные удобства: спа, джакузи, кафе.',
  aquapark: 'Аквапарки Ташкента — горки, аттракционы и волновые бассейны для всей семьи. Отличный вариант для активного отдыха в выходные. Работают в тёплый сезон.',
}

const seoText = computed(() => seoTextMap[slug] ?? category.value?.description ?? '')
