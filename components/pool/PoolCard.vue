<template>
  <NuxtLink :to="cardLink" class="block group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 rounded-2xl">
    <article class="bg-white rounded-2xl shadow-card group-hover:shadow-card-hover transition-all duration-250 overflow-hidden h-full flex flex-col">
      ...
      <div class="relative aspect-[4/3] overflow-hidden shrink-0">
        ...
        >
          <svg class="w-14 h-14 text-white/50" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div v-if="pool.featured" class="absolute top-3 right-3">
          <span class="inline-flex items-center gap-1 bg-amber-400 text-amber-900 text-xs font-bold px-2 py-0.5 rounded-full shadow-sm">
        ...
        <div class="absolute bottom-3 left-3">
          <span class="inline-flex items-center bg-white/90 backdrop-blur-sm text-gray-700 text-xs font-medium px-2.5 py-1 rounded-lg shadow-sm">
      ...
      <div class="p-4 flex flex-col flex-1">
        ...
        <!-- Name -->
        <h3 class="font-semibold text-gray-900 line-clamp-2 leading-snug mb-2 group-hover:text-primary-700 transition-colors duration-150">
          {{ pool.name }}
        </h3>

        <!-- Rating + status -->
        <div class="flex items-center justify-between mb-2">
          <AppRating :rating="pool.rating" :count="pool.reviewCount" size="sm" />
          <PoolStatusBadge :schedule="pool.schedule" />
        </div>

        <!-- District -->
        <div class="flex items-center gap-1.5 text-sm text-gray-400 mb-3">
          <svg class="w-3.5 h-3.5 shrink-0 text-gray-300" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="flex items-center justify-between mt-auto pt-2 border-t border-gray-50">
          <div class="flex items-baseline gap-1">
            ...
            <span class="text-base font-bold text-primary-700">{{ priceDisplay }}</span>
          </div>
          <span
...
const placeholderClass = computed(() => placeholderClasses[props.pool.category])
</script>
